...
Segment::Segment(const std::filesystem::path &dir, uint64_t base_offset, uint64_t max_size, SegmentState state): 
		dir_(dir), 
		...
		state_(state),
		published_size_(0),
		published_offset_(base_offset),
		index_file_(dir, base_offset, state) {
	init();
}

Segment::Segment(const std::filesystem::path &dir, uint64_t base_offset, uint64_t published_offset,uint64_t max_size, SegmentState state) : 
	dir_(dir),
	base_offset_(base_offset),
	max_size_(max_size),
	log_fd_(-1),
	state_(state),
	published_size_(0),
	published_offset_(published_offset),
	index_file_(dir, base_offset, state) {
	init();
}
...
uint64_t Segment::append(const uint8_t *data, uint32_t len)
{
	...
	*/
	...
	uint64_t new_size = published_size_.fetch_add(bytes_written, std::memory_order_release);
	return offset;
}
...
Index::Index(const std::filesystem::path &dir, uint64_t base_offset, SegmentState state): 
	dir_(dir), 
	published_size(0),
	fd_(-1),
	state_(state) {
...
IndexFileEntry Index::determineClosestIndex(uint64_t offset)
{
	...
	if (state_ == SegmentState::Active)
		return binarySearch(offset, reinterpret_cast<const char *>(data_.data()), file_size);
